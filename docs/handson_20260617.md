...
[Please refer to the documentation for more information.](default_databases.md#refseq-and-genbank)
...
- Extra: run with a higher `--max-fp` (e.g. `0.1`) and check the diference in index size. [More infos](default_databases#false-positive-rate)
...
[More infos](default_databases.md#gtdb)
...
- Extra: instead of building by organism group, one can select one or more taxonomic nodes with `--taxid` [More infos](default_databases.md#specific-organisms-or-taxonomic-groups)
...
[More infos](custom_databases/#conversion)
