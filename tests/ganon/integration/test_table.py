...
from tests.ganon.utils import table_sanity_check_and_parse
...
class TestTable(unittest.TestCase):
    ...
    default_params = {
        "input": [
            data_dir + "table/report_reads1.tre",
    ...
    def test_default(self):
        ...
        # Run
        self.assertTrue(
            run_ganon(cfg, params["output_file"]), "ganon table exited with an error"
        )
    ...
    def test_no_rank(self):
        ...
        self.assertIsNotNone(res, "ganon table has inconsistent results")
        self.assertEqual(
            res["out_pd"].columns.values.size, 50, "ganon table without rank failed"
        )
        # Test some outcomes
        self.assertTrue(
            "1|2" in res["out_pd"].columns.values, "ganon table without rank failed"
        )
        self.assertTrue(
        ...
        self.assertEqual(
            res["out_pd"].sum().sum(), 1973568, "ganon table without rank failed"
        )
    ...
    def test_no_rank_unclassified(self):
        ...
        self.assertIsNotNone(res, "ganon table has inconsistent results")
        self.assertEqual(
            res["out_pd"].columns.values.size, 51, "ganon table without rank failed"
        )
        # Test some outcomes
        self.assertTrue(
            "1|2" in res["out_pd"].columns.values, "ganon table without rank failed"
    ...
    def test_no_rank_no_root(self):
        ...
        # should output just bacteria
        self.assertEqual(
            res["out_pd"].columns.values.size, 49, "ganon table without rank failed"
        )
        # Test some outcomes
        self.assertTrue(
            "2" in res["out_pd"].columns.values, "ganon table without rank failed"
        )
        self.assertTrue(
        ...
        self.assertEqual(
            res["out_pd"].sum().sum(), 1703834, "ganon table without rank failed"
        )
    ...
    def test_no_rank_no_root_unclassified(self):
        ...
        self.assertTrue(
            "unclassified" in res["out_pd"].columns.values,
            "ganon table without rank failed",
        )
    ...
    def test_min_count(self):
        ...
        self.assertTrue(
            ((res["out_pd"] == 0) | (res["out_pd"] >= params["min_count"])).all(
                axis=None
            ),
    ...
    def test_max_count(self):
        ...
        # should output just counts higher than min_count (or zeros)
        self.assertTrue(
            ((res["out_pd"] == 0) | (res["out_pd"] <= params["max_count"])).all(
                axis=None
            ),
            "ganon table min count filter failed",
        )
    ...
    def test_taxids_relative(self):
        ...
        self.assertTrue(
            all("Prevotella" in r for r in res["out_pd"].columns.values),
            "ganon table taxids filter failed",
        )
    ...
    def test_names(self):
        ...
        self.assertEqual(
            res["out_pd"].columns.values,
            "Veillonella tobetsuensis",
            "ganon table names filter failed",
        )
    ...
    def test_names_with(self):
        ...
        self.assertTrue(
            all("Prevotella" in r for r in res["out_pd"].columns.values),
            "ganon table names with filter failed",
        )
    ...
    def test_top_sample(self):
        ...
        self.assertEqual(
            res["out_pd"].shape[1], 3, "ganon table top sample filter failed"
        )
    ...
    def test_top_all(self):
        ...
        self.assertEqual(
            res["out_pd"].shape[1], 1, "ganon table top sample filter failed"
        )
    ...
    def test_min_frequency(self):
        ...
        self.assertTrue(
            (res["out_pd"].values > 0).all(), "ganon table min frequency filter failed"
        )
    ...
    def test_extra_cols(self):
        ...
        self.assertTrue(
            all(c in ["UNC", "FIL"] for c in res1["out_pd"].columns.values[-2:]),
        ...
        self.assertFalse(
            "FIL" in res2["out_pd"].columns.values, "ganon table extra cols failed"
        )
        ...
        self.assertFalse(
            "UNC" in res3["out_pd"].columns.values, "ganon table extra cols failed"
        )
        ...
        self.assertTrue(
            all(
                res1["out_pd"][["UNC", "FIL"]].sum(axis=1)
                == res4["out_pd"]["UNASSIGNED"]
            ),
            "ganon table extra cols failed",
        )
    ...
    def test_skip_zeros(self):
        ...
        self.assertEqual(
            res["out_pd"].shape[0], 1, "ganon table skip zeros option failed"
        )
    ...
    def test_header(self):
        ...
        self.assertTrue(
            all(["|" in c for c in res["out_pd"].columns.values]),
