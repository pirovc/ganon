...
def parse_target(target_file):
    ...
    return pd.read_table(
        target_file, sep="\t", header=None, skiprows=0, names=colums, dtype=types
    )
...
def parse_info(info_file):
    ...
    return pd.read_table(
        info_file, sep="\t", header=None, skiprows=0, names=colums, dtype=types
    )
...
def parse_all_one(file):
    ...
    return pd.read_table(
        file, sep="\t", header=None, skiprows=0, names=colums, dtype=types
    )
...
def parse_rep(rep_file):
    ...
    return pd.read_table(
        rep_file, sep="\t", header=None, skiprows=0, names=colums, dtype=types
    )
...
def build_sanity_check_and_parse(params, skipped_targets: bool = False):
    ...
    if not check_files(
        params["db_prefix"], ["ibf"] if params["taxonomy"] == "skip" else ["ibf", "tax"]
    ):
        if not check_files(
            params["db_prefix"],
            ["hibf"] if params["taxonomy"] == "skip" else ["hibf", "tax"],
        ):
            return None
                ...
                if os.path.isdir(i):
                    input_files.extend(
                        list_files_folder(
                            ...
                            recursive=params["input_recursive"],
                        )
                    )
                else:
    ...
    if "input_file" in params and params["input_file"]:
        ...
        if (
            res["info"]["target"].isna().all()
        ):  # in case of one col only, use filename as target
    ...
    if params["taxonomy"] != "skip":
        ...
        if any(
            res["target"]["target"].apply(res["tax"].parent)
            == res["tax"].undefined_node
        ):
    ...
    if params["level"] in ["assembly", "custom"]:  # specializations
        ...
        if (
            not res["info"]["specialization"].isna().all()
            or not res["info"]["specialization_name"].isna().all()
        ):
        ...
        if params[
            "level"
        ]:  # If given and not assembly or custom, should be a taxonomic rank or leaves
...
def report_sanity_check_and_parse(params, sum_full_percentage: bool = True):
    ...
    for file in os.listdir(directory):
        if file.startswith(
            os.path.splitext(os.path.basename(params["output_prefix"]))[0]
        ) and file.endswith(".tre"):
    ...
    for out_tre in output_files:
        ...
        if (
            sum_full_percentage
            and floor(res["tre_pd"][res["idx_base"]]["cumulative_perc"].sum()) != 100
        ):
        ...
        if (
            res["tre_pd"][~res["idx_base"]]["unique"].sum()
            + res["tre_pd"][~res["idx_base"]]["shared"].sum()
        ) > res["tre_pd"][res["idx_base"]]["cumulative"].sum():
        ...
        for rank, val in (
            res["tre_pd"]
            .groupby(by="rank")["cumulative_perc"]
            .sum()
            .apply(floor)
            .items()
        ):
        ...
        for lin in res["tre_pd"]["lineage"]:
            ...
            for node_idx in list(range(len(lineage)))[::-1]:
                ...
                if node_idx and lineage[node_idx] in target_perc:
                    ...
                    if (
                        parent in target_perc
                        and target_perc[lineage[node_idx]] > target_perc[parent]
                    ):
        ...
        for file, vals in input_file.items():
            if sequence:
                for seq in list_sequences([vals["file"]]):
                    vals["target"] = seq
                    print(
                        "\t".join([v for k, v in vals.items() if k in cols]), file=outf
                    )
            else:
