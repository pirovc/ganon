...
std::optional< Config > CommandLineParser::parse( int argc, char** argv )
{
    ...
    options.add_options()
        ...
        ( "o,output-prefix", "Output prefix for final output (prefix.lca) and report (prefix.rep). One for each hierarchy level (prefix.hiearchy.lca, prefix.hiearchy.rep). Omit for output to STDOUT (only .lca)", cxxopts::value< std::string >() )
