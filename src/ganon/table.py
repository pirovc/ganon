...
def parse_tre(file, cfg):
    ...
    for line in open(file):
        ...
        rank, taxid, taxid_lineage, name, _, _, read_count, percentage = line.rstrip().split("\t")
        ...
        if rank == "unclassified" and not cfg.ignore_unclassified_all:
            ...
            if read_count>=cfg.min_matches and percentage>=cfg.min_perc_matches:
    ...
    elif cfg.approximate_names:
        tre, classified_read_count, filtered_read_count = filter_names(tre, cfg.approximate_names, True)
        classified_rank_reads = classified_read_count # restart to count
        filtered_rank_reads+=filtered_read_count # just add elements removed from classified
...
def filter_names(tre, names, approximate=False):
    selected_names = dict()
