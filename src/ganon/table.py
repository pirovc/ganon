...
def table(cfg):
    ...
    if len(root_node) > 1:
        print_log(
            "ERROR: input files should share the same root node, but "
            + str(len(root_node))
            + " root nodes were found: ,".join(root_node),
            cfg.quiet,
        )
    else:
    ...
    if total_taxa - filtered_total_taxa:
        print_log(
            " - Skipped "
            + str(total_taxa - filtered_total_taxa)
            + " taxa with filters",
            cfg.quiet,
        )
    ...
    if cfg.top_sample:
        ...
        print_log(
            " - Skipped "
            + str(filtered_total_taxa - top_sample_total_taxa)
            + " taxa (--top-sample "
            + str(cfg.top_sample)
            + ")",
            cfg.quiet,
        ...
        print_log(
            " - Skipped "
            + str(filtered_total_taxa - top_all_total_taxa)
            + " taxa (--top-all "
            + str(cfg.top_all)
            + ")",
            cfg.quiet,
    ...
    if cfg.min_frequency:
        ...
        print_log(
            " - Skipped "
            + str(filtered_total_taxa - min_frequency_total_taxa)
            + " taxa (--min-frequency "
            + str(cfg.min_frequency)
            + ")",
            cfg.quiet,
        )
    ...
    if not filtered_total_taxa:
        ...
        if cfg.skip_zeros:
            ...
            if len(out_table[0]) < c:
                print_log(
                    " - Skipped "
                    + str(c - len(out_table[0]))
                    + " taxa with only zero counts",
                    cfg.quiet,
                )
        ...
        print_log(
            " - " + str(lines) + "x" + str(cols) + " table saved to " + cfg.output_file,
            cfg.quiet,
        )
...
def parse_reports(tre_files, rank):
    ...
    for tre_file in tre_files:
        reports[tre_file] = {}
        count, lineage, name, total, unclassified, root_node = parse_tre_rank(
            tre_file, rank
        )
    ...
    for file, rep in reports.items():
        for taxid in list(rep["count"]):
            ...
            if cfg.taxids and not any(t in cfg.taxids for t in rep["lineage"][taxid]):
                filtered = True
            elif cfg.names and rep["name"][taxid] not in cfg.names:
                filtered = True
            elif cfg.names_with and not any(
                n in rep["name"][taxid] for n in cfg.names_with
            ):
                filtered = True
...
def select_top_sample(reports, top_sample, root_node):
    ...
    for file, rep in reports.items():
        ...
        for taxid, count in sorted(
            rep["count"].items(), key=lambda x: x[1], reverse=True
        ):  # sorted by count
...
def select_top_all(reports, top_all, root_node):
    ...
    i = 0
    for taxid in sorted(
        total_counts, key=lambda kv: total_counts[kv]["sum_percentage"], reverse=True
    ):
        if taxid == root_node:  # do not count root as an top entry
            continue
        elif i < top_all:
...
def adjust_counts_ranks(reports, no_root, root_node):
    ...
    for file, rep in reports.items():
        ...
        for t in sorted(
            rep["lineage"], key=lambda k: len(rep["lineage"][k]), reverse=True
        ):
