...
def get_sequence_info(cfg, info, tax, tmp_output_folder, use_spec_target):
    if cfg.taxonomy=="ncbi":
        ...
        if mode[0]=="eutils":
            ...
            info.update(run_eutils(cfg, info, tmp_output_folder, skip_taxid=False if tax else True, specialization=cfg.level))
            print_log(" - done in " + str("%.2f" % (time.time() - tx)) + "s.\n", cfg.quiet)
        else:
            if tax:
                acc2txid_urls = []
...
def run_eutils(cfg, info, tmp_output_folder, skip_taxid: bool = False, specialization: str = ""):
    ...
    if specialization in ["assembly", "name"]:
        ...
        if skip_taxid:
            return pd.read_csv(StringIO(stdout), sep="\t", names=["target", "specialization"], index_col="target", header=None, dtype="str", na_values="na")
        else:
            return pd.read_csv(StringIO(stdout), sep="\t", names=["target","length","node","specialization"], index_col="target", header=None, usecols=["target", "node", "specialization"], dtype="str", na_values="na")
    else:
        # return target, taxid
        return pd.read_csv(StringIO(stdout), sep="\t", names=["target","length","node"], index_col="target", header=None, usecols=["target", "node"], dtype="str", na_values="na")
