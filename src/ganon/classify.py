...
def classify(cfg):
    ...
    tax_files = ",".join(tax_files) if len(
        tax_files) == len(filter_files) else ""
    filter_files = ",".join(filter_files)

    run_ganon_classify = " ".join([cfg.path_exec['classify'],
                                   "--single-reads " +
                                   ",".join(
                                       cfg.single_reads) if cfg.single_reads else "",
                                   "--paired-reads " +
                                   ",".join(
        cfg.paired_reads) if cfg.paired_reads else "",
        "--ibf " + filter_files,
        "--tax " + tax_files if tax_files else "",
        "--hierarchy-labels " +
        ",".join(
        cfg.hierarchy_labels) if cfg.hierarchy_labels else "",
        "--rel-cutoff " +
        ",".join(
        [str(rc) for rc in cfg.rel_cutoff]) if cfg.rel_cutoff else "",
        "--rel-filter " +
        ",".join(
        [str(rf) for rf in cfg.rel_filter]) if cfg.rel_filter else "",
        ...
        "--threads " +
        str(cfg.threads) if cfg.threads else "",
        "--n-reads " +
        str(cfg.n_reads) if cfg.n_reads is not None else "",
        "--n-batches " +
