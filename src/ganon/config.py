...
class Config:
    ...
    def __init__(self, which: str=None, **kwargs):
        ...
        classify_group_postrep.add_argument("--skip-report",            action="store_true",                                          help="Disable tree-like report at the end of classification. Can be done later with 'ganon report'.")
        ...
        classify_group_output.add_argument("--output-one",                action="store_true",               help="Output a file with one match for each read (.one) either unique or solved from an EM or a LCA algorithm (--multiple-matches)")
        ...
        classify_group_other.add_argument("-b", "--binning",             action="store_true",                                 help="Optimized parameters for binning (--rel-cutoff 0.25 --rel-filter 0 --report-type reads). Will report sequence abundances (.tre) instead of tax. abundance.")
