...
class Config:
    ...
    def __init__(self, which: str=None, **kwargs):
        ...
        classify_group_postrep.add_argument("-m", "--multiple-matches", type=str,                    metavar="", default="em",        help="Method to solve reads with multiple matches. em -> expectation maximization algorithm based on unique matches. lca -> lowest common ancestor based on taxonomy. The EM algorithm can be done later with 'ganon reassign' (requires .all file with --output-all). [" + ", ".join(self.choices_multiple_matches) + "]", choices=self.choices_multiple_matches)
        ...
        classify_group_postrep.add_argument("--min-count",              type=int_or_float(minval=0), metavar="", default=0.00005,     help="Minimum number/percentage of counts to keep an taxa [values between 0-1 for percentage, >1 specific number]")
        classify_group_postrep.add_argument("--report-type",            type=str,                    metavar="", default="abundance", help="Type of report [" + ", ".join(self.choices_report_type) + "]. More info in 'ganon report'.", choices=self.choices_report_type)
        classify_group_postrep.add_argument("--skip-report",            action="store_true",                                          help="Disable tree-like report at the end of run. Can be done later with 'ganon report'.")
        ...
        classify_group_output.add_argument("--output-one",                action="store_true",               help="Output a file with one match for each read (.one) from an EM or LCA algorithm (--multiple-matches)")
        ...
        classify_group_other.add_argument("-b", "--binning",             action="store_true",                                 help="Optimized parameters for binning (--rel-cutoff 0.25 --rel-filter 0). Will report sequence abundances (.tre) instead of tax. abundance. This file can be re-generated with 'ganon report'.")
