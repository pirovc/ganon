...
class Config:
    ...
    def __init__(self, which: str = None, **kwargs):
        ...
        parser.add_argument(
            ...
            "--version",
            action="version",
            version="version: %(prog)s " + self.version,
        ...
        build_default_required_args = build_default_parser.add_argument_group(
            "required arguments"
        )
        build_default_required_args.add_argument(
        ...
        build_default_advanced_args.add_argument(
            ...
            metavar="",
            default="hibf",
            help="Variant of bloom filter to use ["
            + ", ".join(self.choices_filter_type)
            + "]. hibf requires raptor >= v3.0.1 installed or binary path set with --raptor-path. --mode, --filter-size and --min-length will be ignored with hibf. hibf will set --max-fp 0.001 as default.",
            choices=self.choices_filter_type,
        )
        ...
        build_required_args.add_argument(
            "-g",
            "--organism-group",
            type=str,
            nargs="*",
            metavar="",
            help="One or more organism groups to download ["
            + ", ".join(self.choices_og)
            + "]. Mutually exclusive --taxid",
            choices=self.choices_og,
        )
        build_required_args.add_argument(
            "-a",
            "--taxid",
            ...
            nargs="*",
            metavar="",
            help="One or more taxonomic identifiers to download. e.g. 562 (-x ncbi) or 's__Escherichia coli' (-x gtdb). Mutually exclusive --organism-group",
        )

        build_database_args = build_parser.add_argument_group("database arguments")
        build_database_args.add_argument(
            "-l",
            "--level",
            type=str,
            default="species",
        ...
        build_download_args.add_argument(
            ...
            default=["refseq"],
            metavar="",
            help="Source to download [" + ", ".join(self.choices_db_source) + "]",
            choices=self.choices_db_source,
        )
        build_download_args.add_argument(
            "-o",
            "--top",
            ...
            default=0,
            metavar="",
            help="Download limited assemblies for each taxa. 0 for all.",
        )
        build_download_args.add_argument(
            "-c",
            "--complete-genomes",
        ...
        build_download_args.add_argument(
            ...
            nargs="*",
            metavar="",
            help="Specific files for genome size estimation - otherwise files will be downloaded",
        )
        build_download_args.add_argument(
            "--skip-genome-size",
            action="store_true",
        ...
        build_custom_required_args.add_argument(
            "-i",
            "--input",
            type=str,
            nargs="*",
            metavar="",
            help="Input file(s) and/or folder(s). Mutually exclusive --input-file.",
        )
        build_custom_required_args.add_argument(
            "-e",
            "--input-extension",
        ...
        build_custom_required_args.add_argument(
            ...
            "--input-recursive",
            action="store_true",
            help="Look for files recursively in folder(s) provided with --input",
        )
        ...
        build_custom_args.add_argument(
            ...
            type=file_exists,
            metavar="",
            help="Tab-separated file with all necessary file/sequence information. Fields: file [<tab> target <tab> node <tab> specialization <tab> specialization name]. For details: https://pirovc.github.io/ganon/custom_databases/. Mutually exclusive --input",
        )
        build_custom_args.add_argument(
            "-a",
            "--input-target",
            ...
            default="file",
            metavar="",
            help="Target to use [file, sequence]. Parse input by file or by sequence. Using 'file' is recommended and will speed-up the building process",
            choices=self.choices_input_target,
        )
        build_custom_args.add_argument(
            "-l",
            "--level",
            type=str,
            metavar="",
            help="Max. level to build the database. By default, --level is the --input-target. Options: any available taxonomic rank [species, genus, ...] or 'leaves' (requires --taxonomy). Further specialization options ["
            + ", ".join(self.choices_level)
        ...
        ncbi_args.add_argument(
            ...
            default=[],
            metavar="",
            help="Uses NCBI e-utils webservices or downloads accession2taxid files to extract target information. ["
            + ", ".join(self.choices_ncbi_sequence_info)
            + " or one or more accession2taxid files from https://ftp.ncbi.nlm.nih.gov/pub/taxonomy/accession2taxid/]. By default uses e-utils up-to 50000 sequences or downloads nucl_gb nucl_wgs otherwise.",
        )
        ncbi_args.add_argument(
            "-q",
            "--ncbi-file-info",
            type=str,
            nargs="*",
            default=self.choices_ncbi_file_info[0:2],
            metavar="",
            help="Downloads assembly_summary files to extract target information. ["
            + ", ".join(self.choices_ncbi_file_info)
            + " or one or more assembly_summary files from https://ftp.ncbi.nlm.nih.gov/genomes/]",
        )
        ...
        update_group_required.add_argument(
            "-d",
            "--db-prefix",
            type=str,
            required=True,
        ...
        build_update_other_args = build_update_parser.add_argument_group(
            "optional arguments"
        )
        build_update_other_args.add_argument(
            "--restart",
            action="store_true",
            help="Restart build/update from scratch, do not try to resume from the latest possible step. {db_prefix}_files/ will be deleted if present.",
        )
        build_update_other_args.add_argument(
            "--verbose", action="store_true", help="Verbose output mode"
        )
        build_update_other_args.add_argument(
            "--quiet", action="store_true", help="Quiet output mode"
        )
        build_update_other_args.add_argument(
            "--keep-files", action="store_true", help=argparse.SUPPRESS
        )
        build_update_other_args.add_argument(
            "--write-info-file",
            action="store_true",
            help="Save copy of target info generated to {db_prefix}.info.tsv. Can be re-used as --input-file for further attempts.",
        )
        build_update_other_args.add_argument(
            "--ganon-path", type=str, metavar="", default="", help=argparse.SUPPRESS
        )
        build_update_other_args.add_argument(
            "--raptor-path", type=str, metavar="", default="", help=argparse.SUPPRESS
        )
        build_update_other_args.add_argument(
            "--n-refs", type=unsigned_int(minval=1), metavar="", help=argparse.SUPPRESS
        )
        build_update_other_args.add_argument(
            "--n-batches",
            type=unsigned_int(minval=1),
        ...
        build_update_other_args.add_argument(
            ...
            type=str,
            metavar="",
            default="https://ftp.ncbi.nlm.nih.gov/",
            help=argparse.SUPPRESS,
        )
        build_update_other_args.add_argument(
            "--gtdb-url",
            type=str,
            metavar="",
            default="https://data.gtdb.ecogenomic.org/releases/latest/",
            help=argparse.SUPPRESS,
        )
        ...
        # Required
        classify_group_required = classify_parser.add_argument_group(
            "required arguments"
        )
        ...
        classify_group_required.add_argument(
            ...
            required=False,
            metavar="reads.fq[.gz]",
            help="Multi-fastq[.gz] file[s] to classify",
        )
        classify_group_required.add_argument(
            "-p",
            "--paired-reads",
        ...
        classify_group_cutoff_filter.add_argument(
            "-c",
            "--rel-cutoff",
            type=int_or_float(minval=0, maxval=1),
            nargs="*",
            metavar="",
            default=[0.75],
            help="Min. percentage of a read (set of k-mers) shared with a reference necessary to consider a match. Generally used to remove low similarity matches. Single value or one per database (e.g. 0.7 1 0.25). 0 for no cutoff",
        )
        classify_group_cutoff_filter.add_argument(
            "-e",
            "--rel-filter",
            ...
            metavar="",
            default=[0.1],
            help="Additional relative percentage of matches (relative to the best match) to keep. Generally used to keep top matches above cutoff. Single value or one per hierarchy (e.g. 0.1 0). 1 for no filter",
        )

        classify_group_postrep = classify_parser.add_argument_group(
        ...
        classify_group_postrep.add_argument(
            ...
            metavar="",
            default="em",
            help="Method to solve reads with multiple matches  ["
            + ", ".join(self.choices_multiple_matches)
            + "]. em -> expectation maximization algorithm based on unique matches. lca -> lowest common ancestor based on taxonomy. The EM algorithm can be executed later with 'ganon reassign' using the .all file (--output-all).",
            choices=self.choices_multiple_matches,
        )
        classify_group_postrep.add_argument(
            "--ranks",
            type=str,
        ...
        classify_group_postrep.add_argument(
            "--min-count",
            type=int_or_float(minval=0),
            metavar="",
            default=0.00005,
            help="Minimum percentage/counts to report an taxa (.tre) [use values between 0-1 for percentage, >1 for counts]",
        )
        classify_group_postrep.add_argument(
            "--report-type",
            type=str,
            metavar="",
            default="abundance",
            help="Type of report (.tre) ["
            + ", ".join(self.choices_report_type)
            + "]. More info in 'ganon report'.",
            choices=self.choices_report_type,
        )
        classify_group_postrep.add_argument(
            "--skip-report",
            action="store_true",
            help="Disable tree-like report (.tre) at the end of classification. Can be done later with 'ganon report'.",
        )
        ...
        classify_group_output.add_argument(
            "--output-one",
            action="store_true",
            help="Output a file with one match for each read (.one) either an unique match or a result from the EM or a LCA algorithm (--multiple-matches)",
        )
        classify_group_output.add_argument(
            "--output-all",
            action="store_true",
            help="Output a file with all unique and multiple matches (.all)",
        )
        classify_group_output.add_argument(
            "--output-unclassified",
            action="store_true",
        ...
        classify_group_other.add_argument(
            "-t",
            "--threads",
            type=unsigned_int(minval=1),
            metavar="",
        ...
        classify_group_other.add_argument(
            ...
            "--binning",
            action="store_true",
            help="Optimized parameters for binning (--rel-cutoff 0.25 --rel-filter 0 --min-count 0 --report-type reads). Will report sequence abundances (.tre) instead of tax. abundance.",
        )
        classify_group_other.add_argument(
            "-f",
            "--fpr-query",
            ...
            metavar="",
            default=[1e-5],
            help="Max. false positive of a query to accept a match. Applied after --rel-cutoff and --rel-filter. Generally used to remove false positives matches querying a database build with large --max-fp. Single value or one per hierarchy (e.g. 0.1 0). 1 for no filter",
        )
        classify_group_other.add_argument(
            "-l",
            "--hierarchy-labels",
            ...
            nargs="*",
            metavar="",
            help="Hierarchy definition of --db-prefix files to be classified. Can also be a string, but input will be sorted to define order (e.g. 1 1 2 3). The default value reported without hierarchy is 'H1'",
        )
        classify_group_other.add_argument(
            "--verbose", action="store_true", help="Verbose output mode"
        )
        ...
        classify_group_other.add_argument(
            "--hibf", action="store_true", help=argparse.SUPPRESS
        )
        classify_group_other.add_argument(
            "--ganon-path", type=str, default="", metavar="", help=argparse.SUPPRESS
        )
        classify_group_other.add_argument(
            "--n-reads", type=unsigned_int(minval=1), metavar="", help=argparse.SUPPRESS
        )
        classify_group_other.add_argument(
            "--n-batches",
            type=unsigned_int(minval=1),
        ...
        # Required
        reassign_group_required = reassign_parser.add_argument_group(
            "required arguments"
        )
        ...
        reassign_em.add_argument(
            "-e",
            "--max-iter",
            type=unsigned_int(minval=0),
            metavar="",
            default=10,
            help="Max. number of iterations for the EM algorithm. If 0, will run until convergence (check --threshold)",
        )
        reassign_em.add_argument(
            "-s",
            "--threshold",
            ...
            metavar="",
            default=0,
            help="Convergence threshold limit to stop the EM algorithm.",
        )
        ...
        reassign_group_other.add_argument(
            "--remove-all",
            action="store_true",
            help="Remove input file (.all) after processing.",
        )
        reassign_group_other.add_argument(
            "--skip-one",
            action="store_true",
        ...
        report_group_required.add_argument(
            ...
            nargs="*",
            metavar="",
            help="Input file(s) and/or folder(s). '.rep' file(s) from ganon classify.",
        )
        report_group_required.add_argument(
            "-e",
            "--input-extension",
        ...
        report_group_dbtax.add_argument(
            ...
            metavar="",
            default=[],
            help="Database prefix(es) used for classification. Only '.tax' file(s) are required. If not provided, new taxonomy will be downloaded. Mutually exclusive with --taxonomy.",
        )
        report_group_dbtax.add_argument(
            "-x",
            "--taxonomy",
            ...
            + ", ".join(self.choices_taxonomy)
            + "]. Mutually exclusive with --db-prefix.",
            choices=self.choices_taxonomy,
        )
        report_group_dbtax.add_argument(
            "-m",
        ...
        report_group_dbtax.add_argument(
            "--skip-genome-size",
            action="store_true",
            help="Do not attempt to get genome sizes. Valid only without --db-prefix. Activate this option when using sequences not representing full genomes.",
        )
        ...
        report_group_output.add_argument(
            ...
            default="tsv",
            help="Output format ["
            + ", ".join(self.choices_report_output)
            + "]. text outputs a tabulated formatted text file for better visualization. bioboxes is the the CAMI challenge profiling format (only percentage/abundances are reported).",
            choices=self.choices_report_output,
        )
        report_group_output.add_argument(
            "-t",
            "--report-type",
        ...
        report_group_output.add_argument(
            ...
            metavar="",
            default="",
            help="Sort report by [rank, lineage, count, unique]. Default: rank (with custom --ranks) or lineage (with --ranks all)",
        )
        report_group_output.add_argument(
            "-a",
            "--no-orphan",
            action="store_true",
            help="Omit orphan nodes from the final report. Otherwise, orphan nodes (= nodes not found in the db/tax) are reported as 'na' with root as direct parent.",
        )
        report_group_output.add_argument(
            "-y",
            "--split-hierarchy",
            action="store_true",
            help="Split output reports by hierarchy (from ganon classify --hierarchy-labels). If activated, the output files will be named as '{output_prefix}.{hierarchy}.tre'",
        )
        report_group_output.add_argument(
            "-p",
            "--skip-hierarchy",
            ...
            metavar="",
            default=[],
            help="One or more hierarchies to skip in the report (from ganon classify --hierarchy-labels)",
        )
        report_group_output.add_argument(
            "-k",
            "--keep-hierarchy",
            ...
            metavar="",
            default=[],
            help="One or more hierarchies to keep in the report (from ganon classify --hierarchy-labels)",
        )
        report_group_output.add_argument(
            "-c",
            "--top-percentile",
            type=int_or_float(minval=0, maxval=0.999999),
            metavar="",
            default=0,
        ...
        report_group_output.add_argument(
            ...
            "--normalize",
            action="store_true",
            help="Ignore the number of unclassified reads, normalizing the output to 100%%. Use with caution, can drastically change abundance estimations.",
        )

        report_group_optional = report_parser.add_argument_group("optional arguments")
        report_group_optional.add_argument(
            "--verbose", action="store_true", default=False, help="Verbose output mode"
        )
        report_group_optional.add_argument(
            "--quiet", action="store_true", default=False, help="Quiet output mode"
        )
        report_group_optional.add_argument(
            "--ncbi-url",
            type=str,
        ...
        table_group_required.add_argument(
            "-i",
            "--input",
            type=str,
            required=True,
            nargs="*",
            metavar="",
            help="Input file(s) and/or folder(s). '.tre' file(s) from ganon report.",
        )
        table_group_required.add_argument(
            "-e",
            "--input-extension",
        ...
        table_group_required.add_argument(
            ...
            type=str,
            required=True,
            help="Output filename for the table",
        )

        table_group_output = table_parser.add_argument_group("output arguments")
        table_group_output.add_argument(
            "-l",
            "--output-value",
            type=str,
            metavar="",
            default="counts",
            help="Output value on the table [percentage, counts]. percentage values are reported between [0-1]",
        )
        table_group_output.add_argument(
            "-f",
            "--output-format",
        ...
        table_group_output.add_argument(
            ...
            metavar="",
            default=0,
            help="Top hits of each sample individually",
        )
        table_group_output.add_argument(
            "-a",
            "--top-all",
            ...
            metavar="",
            default=0,
            help="Top hits of all samples (ranked by percentage)",
        )
        table_group_output.add_argument(
            "-m",
            "--min-frequency",
            ...
            metavar="",
            default=0,
            help="Minimum number/percentage of files containing an taxa to keep the taxa [values between 0-1 for percentage, >1 specific number]",
        )
        table_group_output.add_argument(
            "-r",
            "--rank",
            ...
            metavar="",
            default=None,
            help="Define specific rank to report. Empty will report all ranks.",
        )
        table_group_output.add_argument(
            "-n",
            "--no-root",
            action="store_true",
            default=False,
            help="Do not report root node entry and lineage. Direct and shared matches to root will be accounted as unclassified",
        )
        table_group_output.add_argument(
            "--header",
            type=str,
            metavar="",
            default="name",
            help="Header information [name, taxid, lineage]",
        )
        table_group_output.add_argument(
            "--unclassified-label",
            type=str,
            metavar="",
            default=None,
            help="Add column with unclassified count/percentage with the chosen label. May be the same as --filtered-label (e.g. unassigned)",
        )
        table_group_output.add_argument(
            "--filtered-label",
            type=str,
            metavar="",
            default=None,
            help="Add column with filtered count/percentage with the chosen label. May be the same as --unclassified-label (e.g. unassigned)",
        )
        table_group_output.add_argument(
            "--skip-zeros",
            action="store_true",
            default=False,
            help="Do not print lines with only zero count/percentage",
        )
        table_group_output.add_argument(
            "--transpose",
            action="store_true",
            default=False,
            help="Transpose output table (taxa as cols and files as rows)",
        )

        table_group_optional = table_parser.add_argument_group("optional arguments")
        table_group_optional.add_argument(
            "--verbose", action="store_true", default=False, help="Verbose output mode"
        )
        table_group_optional.add_argument(
            "--quiet", action="store_true", default=False, help="Quiet output mode"
        ...
        filter_arguments.add_argument(
            ...
            metavar="",
            default=0,
            help="Minimum number/percentage of counts to keep an taxa [values between 0-1 for percentage, >1 specific number]",
        )
        filter_arguments.add_argument(
            "--max-count",
            type=int_or_float(minval=0),
            metavar="",
            default=0,
            help="Maximum number/percentage of counts to keep an taxa [values between 0-1 for percentage, >1 specific number]",
        )
        filter_arguments.add_argument(
            "--names",
            type=str,
            ...
            metavar="",
            default=[],
            help="Show only entries matching exact names of the provided list",
        )
        filter_arguments.add_argument(
            "--names-with",
            type=str,
            ...
            metavar="",
            default=[],
            help="Show entries containing full or partial names of the provided list",
        )
        filter_arguments.add_argument(
            "--taxids",
            type=str,
        ...
        build_custom = subparsers.add_parser(
            "build-custom",
    ...
    def validate(self):
        ...
        if self.which == "build":
            ...
            if self.filter_type == "hibf" and self.hash_functions == 0:
                print_log(
                    "--filter-type hibf requires --hash-function value between 1 and 5"
                )
                return False
            ...
            if (
                self.level
                and self.level not in self.choices_level
                and self.taxonomy == "none"
            ):
            ...
            if self.taxonomy == "ncbi":
                for entry in self.ncbi_sequence_info:
                    if entry not in self.choices_ncbi_sequence_info and not check_file(
                        ...
                        print_log(
                            "Invalid --get-sequence-info. Should be a valid file or: "
                            + " ".join(self.choices_ncbi_sequence_info)
                        )
                        return False

                for entry in self.ncbi_file_info:
                    if entry not in self.choices_ncbi_file_info and not check_file(
                        entry
                    ):
                        print_log(
                            "Invalid --get-file-info. Should be a valid file or: "
                            + " ".join(self.choices_ncbi_file_info)
                        )
                        return False
            ...
            if not check_folder(set_output_folder(self.db_prefix)):
                print_log(
                    "Folder to update not found: " + set_output_folder(self.db_prefix)
                )
                return False

            if self.output_db_prefix and check_folder(
                set_output_folder(self.output_db_prefix)
            ):
                print_log(
                    "New output folder already exists and it's not empty: "
                    + set_output_folder(self.output_db_prefix)
                )
                return False

            update_required_files = [
                "_files/assembly_summary.txt",
                "_files/history.tsv",
                "_files/config.pkl",
            ]
            ...
            if hibf and ibf:
                print_log(
                    ".ibf and .hibf filters cannot be used together in the same run"
                )
                return False
            elif hibf:
                # Hidden param
            ...
            if tax < len(self.db_prefix) and tax > 0:
                print_log(
                    ".tax file has to be present for every .ibf/.hibf or none of them"
                )
                return False
            ...
            if self.skip_hierarchy and self.keep_hierarchy:
                print_log(
                    "--skip-hierarchy and --keep-hierarchy are mutually exclusive"
                )
                return False
    ...
    def set_paths(self):
        ...
        if self.which in ["build", "update", "build-custom"]:
            ganon_genome_updater_paths = (
                ...
                if self.ganon_path
                else [None, "libs/genome_updater/"]
            )
            for p in ganon_genome_updater_paths:
                self.path_exec["genome_updater"] = shutil.which(
                    "genome_updater.sh", path=p
                )
                if self.path_exec["genome_updater"] is not None:
                    break
            if self.path_exec["genome_updater"] is None:
                print_log(
                    "genome_updater.sh was not found. Please inform a specific path with --ganon-path"
                )
                missing_path = True

            ganon_get_seq_info_paths = (
                [
                    self.ganon_path,
                    self.ganon_path + "scripts/",
                    self.ganon_path + "../scripts/",
                ]
                if self.ganon_path
                else [None, "scripts/"]
            ...
            for p in ganon_get_seq_info_paths:
                self.path_exec["get_seq_info"] = shutil.which(
                    "ganon-get-seq-info.sh", path=p
                )
                if self.path_exec["get_seq_info"] is not None:
                    break
            if self.path_exec["get_seq_info"] is None:
                print_log(
                    "ganon-get-seq-info.sh script was not found. Please inform a specific path with --ganon-path"
                )
                missing_path = True

            # if path is given, look for binaries only there
            ganon_build_paths = (
                [self.ganon_path, self.ganon_path + "build/"]
                if self.ganon_path
                else [None, "build/"]
            )
            for p in ganon_build_paths:
                self.path_exec["build"] = shutil.which("ganon-build", path=p)
                if self.path_exec["build"] is not None:
                    break
            if self.path_exec["build"] is None:
                print_log(
                    "ganon-build binary was not found. Please inform a specific path with --ganon-path"
                )
                missing_path = True
            ...
            if hasattr(self, "filter_type") and self.filter_type == "hibf":
                ...
                raptor_paths = (
                    [self.raptor_path, self.raptor_path + "build/bin/"]
                    if self.raptor_path
                    else [None, "build/"]
                )
                for p in raptor_paths:
                    self.path_exec["raptor"] = shutil.which("raptor", path=p)
                    if self.path_exec["raptor"] is not None:
                        break
                if self.path_exec["raptor"] is None:
                    print_log(
                        "raptor binary was not found. Please inform a specific path with --raptor-path or use --filter-type ibf"
                    )
                    missing_path = True

        if self.which in ["classify"]:
            ganon_classify_paths = (
                [self.ganon_path, self.ganon_path + "build/"]
                if self.ganon_path
                else [None, "build/"]
            )
            for p in ganon_classify_paths:
                ...
                if self.path_exec["classify"] is not None:
                    break
            if self.path_exec["classify"] is None:
                print_log(
                    "ganon-classify binary was not found. Please inform a specific path with --ganon-path"
                )
                missing_path = True
