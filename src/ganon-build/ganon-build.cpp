...
TInterleavedBloomFilter load_filter(Arguments & args, const std::set< uint64_t > & bin_ids){
    ...
    uint64_t number_of_bins = bin_ids.size();
    // dummy variables in case of update
    if ( !args.update_bloom_filter_file.empty() )
    {
        number_of_bins = args.hash_functions = args.kmer_size = args.bloom_filter_size = 1;
    }

    // define filter
    TInterleavedBloomFilter filter( number_of_bins, args.hash_functions, args.kmer_size, args.bloom_filter_size );

    // load from disk in case of update
    if ( !args.update_bloom_filter_file.empty() )
    {
        // load filter
        seqan::retrieve( filter, seqan::toCString( args.update_bloom_filter_file ) );
        ...
        if ( args.update_complete )
        {
            ...
            ubins.insert( ubins.end(), bin_ids.begin(), bin_ids.end() );
            seqan::clear( filter, ubins, args.threads );
        }
...
int main( int argc, char** argv )
{
    ...
    std::cerr << seq_bin.size() << " sequences, " << bin_ids.size() << " bins" << std::endl;

    // load new or given filter
    TInterleavedBloomFilter filter = load_filter(args, bin_ids);
    
    std::chrono::duration< double > elapsed = std::chrono::high_resolution_clock::now() - start;
    std::cerr << "Loading filter and files: " << elapsed.count() << std::endl;
    ...
    start = std::chrono::high_resolution_clock::now();
    seqan::store( filter, seqan::toCString( args.output_file ) );
    elapsed = std::chrono::high_resolution_clock::now() - start;
