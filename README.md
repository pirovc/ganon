...
### report
 ...
 - {prefix}**.tre**: tab-separated tree-like report with cumulative counts and taxonomic lineage. There are 3 possible report types (`--report-type`): abundance, reads or matches
   - abundance: generated by default, will attempt to estimate **taxonomic abundances** by re-disributing reads with an LCA match and correcting by genome size.
   - reads: **sequence abundances**, reports the proportion of sequences assigned to a taxa, each read classified is counted once
   - matches: every match is reported to their original target, including multiple matches
