...
We suggest using [genome_updater](https://github.com/pirovc/genome_updater) to download sequences from RefSeq/Genbank. genome_updater can keep a subset (organism or taxonomic groups) of those repositories updated. For example:
...
Where `-a` will additionaly download the taxdump.tar.gz, `-m` will force the MD5 check for every file downloaded, `-u -r -p` will generate reports which can be used later, `-t` set the number of parallel downloads and `-o` the output folder. If you want to download a set of defined species instead of the whole organism group, use `-g "species:562,623"` or any taxonomic group(s) `-g "taxids:620,1643685"`.

Building the index based on the files of the example above:

	ganon build --db-prefix ganon_db --input-files RefSeqCG_arc_bac/files/*.genomic.fna.gz
...
Optionally, some extra files generated by genome_updater can be further used to speed-up the building process:
	...
	ganon build --db-prefix ganon_db --input-files ${output_dir}/files/*.genomic.fna.gz --seq-info-file ${output_dir}/${timestamp_download}_seqinfo.txt --taxdump-file ${output_dir}/${timestamp_download}_taxdump.tar.gz
...
This command will check if the last downloaded version is complete and additionaly download the new files (and remove the outdated ones). New log and report files with the current timestamp will be generated.
...
Updating the index based on the files of the example above:
	...
	ganon update --db-prefix ganon_db --output-db-prefix ganon_db_updated --input-files $(grep "genomic.fna.gz$" ${output_dir}/${timestamp_update}_url_list_downloaded.txt | sed 's/.*\//${output_dir}\/files\//' | tr '\n' ' ')

If `--output-db-prefix` is not set, the database files will be overwritten with the updated ones.
...
If `-d genbank` in genome_updater, change the occurances of `$4` to `$3` in the `awk` command above.
