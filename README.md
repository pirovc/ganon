...
We suggest using [genome_updater](https://github.com/pirovc/genome_updater) to download sequences from RefSeq/Genbank. genome_updater can keep a sub-set (organism group or taxonomic ids) of those repositories updated. For example:

Downloading archaeal and bacterial complete genomes from RefSeq: 

	genome_updater.sh -g "archaea,bacteria" -d "refseq" -l "Complete Genome" -f "genomic.fna.gz,assembly_report.txt" -a -m -u -r -p -t 24 -o RefSeqCG_arc_bac

Where `-a` will download the taxdump.tar.gz, `-m` will force the MD5 check for every file, `-u -r -p` will generate reports which can be used later, `-t` set the number of parallel downloads and `-o` the output folder. If you want to download a set of defined species instead of the whole organism group, use `-g "species:562,623"` or any taxonomic groups `-g "taxids:620,1643685"`.

Building the index based on the files of the example above:

	ganon build --db-prefix RefSeqCG_arc_bac --input-files RefSeqCG_arc_bac/files/*.genomic.fna.gz

### Using extra files to build

Optionally, some extra files generated by genome_updater can be further used to speed-up the building process:

	# Set output directory and timestamp used to download files
	output_dir="RefSeqCG_arc_bac"
	timestamp_download="2019-01-01_00-00-00"

	# Extract taxonomic information from genome_updater reports
	awk 'BEGIN {FS="\t";OFS="\t"}{if($4!="na"){ print $4,$5,$6,$2 }}' ${output_dir}/${timestamp_download}_updated_sequence_accession.txt > ${output_dir}/${timestamp_download}_seqinfo.txt
 
	# Use generated files on ganon build
	ganon build --db-prefix RefSeqCG_arc_bac --input-files ${output_dir}/files/*.genomic.fna.gz --seq-info-file ${output_dir}/${timestamp_download}_seqinfo.txt --taxdump-file ${output_dir}/${timestamp_download}_taxdump.tar.gz

`{TIMESTAMP}` stands for the timestamp (YYYY-MM-DD_HH-MM-SS) automatically generated when the files were downloaded. If `-d genbank` in genome_updater, change the occurances of `$4` to `$3` in the `awk` command above.

## Updating sequences

To update the folder with the most recent releases (after some days), just re-run the download command:

	genome_updater.sh -g "archaea,bacteria" -d "refseq" -l "Complete Genome" -f "genomic.fna.gz,assembly_report.txt" -a -m -u -r -p -t 24 -o RefSeqCG_arc_bac

This command will check if the last version is complete and additionaly download the new files (and remove the outdated ones). New files with the current timestamp will be generated.

Updating the index based on the files of the example above:

	# Set output directory and timestamp used to update files
	output_dir="RefSeqCG_arc_bac"
	timestamp_update="2019-06-01_00-00-00"
	ganon update --db-prefix RefSeqCG_arc_bac --output-db-prefix RefSeqCG_arc_bac_updated --input-files $(grep "genomic.fna.gz$" ${output_dir}/${timestamp_update}_url_list_downloaded.txt | sed 's/.*\//${output_dir}\/files\//' | tr '\n' ' ')

If `--output-db-prefix` is not set, the old database files will be overwritten with the updated ones. 

Optionally, some extra files generated by genome_updater can be further used to speed-up the updating process:

	awk 'BEGIN {FS="\t";OFS="\t"}{if($1=="A" && $4!="na"){ print $4,$5,$6,$2 }}' ${output_dir}/${timestamp_update}_updated_sequence_accession.txt > ${output_dir}/${timestamp_update}_seqinfo.txt

	# Use generated files on ganon update
	ganon update --db-prefix RefSeqCG_arc_bac --output-db-prefix RefSeqCG_arc_bac_updated --input-files $(grep "genomic.fna.gz$" ${output_dir}/${timestamp_update}_url_list_downloaded.txt | sed 's/.*\//${output_dir}\/files\//' | tr '\n' ' ') --seq-info-file ${output_dir}/${timestamp_update}_seqinfo.txt --taxdump-file ${output_dir}/${timestamp_update}_taxdump.tar.gz
