...
Obs:
-  If `-d genbank` was used in genome_updater, change the occurances of `$4` to `$3` in the `awk` command above.
- `{TIMESTAMP}` should be the timestamp (YYYY-MM-DD_HH-MM-SS) automatically generated when the files were downloaded inside the `RefSeqCG_arc_bac/v2/`.
