...
### report
 ...
 - {prefix}**.tre**: tab-separated tree-like report with cumulative counts and taxonomic lineage. There are several possible report types (`--report-type`):
   - *abundance*: generated by default, will attempt to estimate **taxonomic abundances** by re-disributing reads assigned with LCA (based on number of unique matches on leaf nodes) and correcting by [approximate] genome size.
   - *reads*: **sequence abundances**, reports the proportion of sequences assigned to a taxa, each read classified is counted once.
   - *dist*: **sequence abundances** with re-redistribution of shared reads (like *abundance*) but without correction by genome size.
